...
function Index() {
  ...
  // Google Review popup — mostra em mobile após 8 segundos (apenas uma vez por sessão)
  useEffect(() => {
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    const alreadySeen = sessionStorage.getItem("review-popup-seen");
    ...
    <div className="min-h-screen bg-black text-amber-50 font-sans relative">
      ...
      {/* Contactos / Footer */}
        <div className="max-w-5xl mx-auto px-6">
          ...
          {/* Título */}
          <h2 className="font-serif text-5xl md:text-6xl text-center text-amber-400 mb-6">
            {lang === "pt" ? "PORQUE NOS ESCOLHER" : "WHY CHOOSE US"}
          </h2>
          <p className="text-amber-200/60 text-center text-sm tracking-[0.2em] uppercase mb-16">
            {lang === "pt" ? "O melhor steakhouse do Porto" : "Porto's finest steakhouse"}
          </p>
            ...
            {[
              {
                ...
                title: lang === "pt" ? "Brasa Viva" : "Live Grill",
                text: lang === "pt" ? "Os nossos cortes são preparados na brasa viva, garantindo o sabor e a textura perfeitos em cada prato." : "Our cuts are prepared on a live grill, ensuring perfect flavor and texture in every dish.",
              },
              {
                icon: (
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                title: lang === "pt" ? "Cortes Premium" : "Premium Cuts",
                text: lang === "pt" ? "Selecionamos os melhores cortes — Tomahawk, T-Bone, Picanha Black Angus — para uma experiência gastronómica única." : "We select the finest cuts — Tomahawk, T-Bone, Black Angus Picanha — for a unique gastronomic experience.",
              },
              {
                icon: (
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                title: lang === "pt" ? "Jantares de Grupo" : "Group Dinners",
                text: lang === "pt" ? "O espaço ideal para jantares de grupo e empresa no Porto. Uma experiência memorável para toda a equipa." : "The ideal space for group and corporate dinners in Porto. A memorable experience for the whole team.",
              },
            ].map((item, i) => (
              <div key={i} className="border border-amber-900/30 p-6 bg-black/40 backdrop-blur-sm hover:border-amber-600/50 transition-all duration-500 group text-center">
                <div className="w-16 h-16 mx-auto mb-4 border border-amber-600/50 rounded-full flex items-center justify-center text-amber-400 group-hover:scale-110 transition-transform">
                  {item.icon}
                </div>
                <h3 className="font-serif text-xl text-amber-400 mb-3 tracking-wider">{item.title}</h3>
                <p className="text-amber-200/60 text-sm leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="flex items-center justify-center gap-3 mb-10">
              ...
              <h3 className="font-serif text-2xl text-amber-400 tracking-wider">
                {lang === "pt" ? "O QUE DIZEM OS NOSSOS CLIENTES" : "WHAT OUR GUESTS SAY"}
              </h3>
            </div>

            <div className="grid md:grid-cols-3 gap-6">
              {[
                { name: "João M.", text: lang === "pt" ? "Uma experiência incrível! O Tomahawk estava perfeito e o ambiente é simplesmente espetacular. Voltarei certamente." : "An incredible experience! The Tomahawk was perfect and the atmosphere is simply spectacular. I will definitely return.", date: "Janeiro 2025" },
                { name: "Ana S.", text: lang === "pt" ? "O melhor steakhouse do Porto, sem dúvida. Cortes de qualidade excepcional e um serviço irrepreensível. Recomendo a todos!" : "The best steakhouse in Porto, without a doubt. Exceptional quality cuts and impeccable service. I recommend it to everyone!", date: "Fevereiro 2025" },
                { name: "Carlos R.", text: lang === "pt" ? "Fizemos um jantar de grupo aqui e foi memorável. O espaço, a comida e a atenção ao detalhe fazem deste lugar algo único no Porto." : "We had a group dinner here and it was memorable. The space, food and attention to detail make this place unique in Porto.", date: "Março 2025" },
              ].map((review, i) => (
                <div key={i} className="border border-amber-900/30 p-6 bg-black/40 backdrop-blur-sm hover:border-amber-600/30 transition-all duration-500">
                  {/* Stars */}
                  ...
                  <p className="text-amber-200/70 text-sm leading-relaxed mb-4 italic">"{review.text}"</p>
                  <div className="flex justify-between items-center">
                    <span className="text-amber-400 text-sm font-medium">{review.name}</span>
                    <span className="text-amber-200/30 text-xs">{review.date}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center mt-8">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-amber-400/70 hover:text-amber-400 transition-colors text-sm tracking-wider"
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        ...
        <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center px-4 pb-6 sm:pb-0">
          ...
          <div className="relative w-full max-w-sm bg-[#111] border border-amber-600/40 p-6 shadow-2xl animate-fade-in">
            ...
            <p className="text-amber-200/70 text-sm text-center mb-5 leading-relaxed">
              A sua opinião faz toda a diferença. Deixe-nos uma avaliação no Google — leva apenas 1 minuto!
            </p>
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setShowReviewPopup(false)}
              className="block w-full text-center py-3 bg-amber-400 text-black font-medium tracking-[0.15em] text-sm hover:bg-amber-300 transition-colors"
            >
              AVALIAR AGORA
            </a>
